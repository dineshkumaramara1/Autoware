...
VelodyneDecoder::VelodyneDecoder(
  ...
  sweep_data(new velodyne_msgs::VelodyneSweep()){
  return;
}

bool VelodyneDecoder::loadParameters() {
...
bool VelodyneDecoder::createRosIO() {
  packet_sub = nh.subscribe<velodyne_msgs::VelodynePacket>(
      "velodyne_packet", 100, &VelodyneDecoder::packetCallback, this);
  sweep_pub = nh.advertise<velodyne_msgs::VelodyneSweep>(
      "velodyne_sweep", 10);
...
bool VelodyneDecoder::setupDiagnostics() {
  ROS_INFO("expected frequency: %.3f (Hz)", frequency);
  using namespace diagnostic_updater;
  diagnostics_.setHardwareID("Velodyne Decoder");
  diag_topic_.reset(new TopicDiagnostic("sweep/pointcloud", diagnostics_,
                                        FrequencyStatusParam(&frequency,
                                                             &frequency,
                                                             0.1, 10),
                                        TimeStampStatusParam()));
  diag_timer = nh.createTimer(ros::Duration(1.0), &VelodyneDecoder::daignosticUpdateCallback,this);
  return true;
}

void VelodyneDecoder::daignosticUpdateCallback(const ros::TimerEvent& te)
{
  diagnostics_.update();
}

bool VelodyneDecoder::initialize() {
  if (!loadParameters()) {
  ...
  if (!setupDiagnostics()) {
    ROS_ERROR("Cannot create Diagnostics...");
    return false;
  }
...
bool VelodyneDecoder::checkPacketValidity(const RawPacket* packet) {
...
void VelodyneDecoder::publishPointCloud() {
  ...
  for (size_t i = 0; i < 16; ++i) {
    const velodyne_msgs::VelodynePointScan& scan = sweep_data->scans[i];
...
void VelodyneDecoder::decodePacket(const RawPacket* packet) {
...
void VelodyneDecoder::packetCallback(
    const velodyne_msgs::VelodynePacketConstPtr& msg) {
  ...
  for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
    for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
      ...
      sweep_data->scans[remapped_scan_idx].points.push_back(
          velodyne_msgs::VelodynePoint());
      velodyne_msgs::VelodynePoint& new_point =
  ...
  if (end_fir_idx != FIRINGS_PER_PACKET) {
    ...
    diag_topic_->tick(sweep_data->header.stamp);
    diagnostics_.update();

    sweep_data = velodyne_msgs::VelodyneSweepPtr(
        new velodyne_msgs::VelodyneSweep());
